<template>
  <div class="w-full h-dvh flex justify-center items-center">
    <div
      class="scroll-bar mx-auto xl:max-w-[1100px] flex xl:justify-center w-full overflow-x-auto p-3 rounded-2xl"
      ref="tableContainer"
      @scroll="handleScroll"
    >
      <table class="bg-white border border-gray-200">
        <thead>
          <tr>
            <th class="py-2 px-5 border-b border-b-[#edeff3]">
          ...
          <tr v-for="row in rows" :key="row.id">
            ...
            <td class="pr-8 py-5 whitespace-nowrap text-sm text-gray-500">
              <div class="flex flex-row items-center gap-3 w-max">
                <div
                  class="w-[50px] h-[50px] rounded-md flex justify-center items-center bg-[#F7F8F9]"
                >
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <div v-if="row.highlight" class="flex items-end gap-1">
                    <svg
                    ...
                    >
                      <g>
                        <path
                          d="M155.109 74.028a4 4 0 0 0-3.48-2.028h-52.4l8.785-67.123a4.023 4.023 0 0 0-7.373-2.614L36.917 113.905A4 4 0 0 0 40.324 120h51.617l-6.962 67.224a4.024 4.024 0 0 0 7.411 2.461l62.671-111.63a4 4 0 0 0 .048-4.027z"
                          opacity="1"
                          data-original="#0C66E4"
                          :class="row.status === 'აქტიური' ? 'fill-[#0C66E4]' : 'fill-[#6E5DC6]'"
                        ></path>
                      </g>
                    </svg>
                    <span
                      class="text-sm font-semibold leading-none pb-[3px]"
                      :class="row.status === 'აქტიური' ? 'text-[#0C66E4]' : 'text-[#6E5DC6]'"
                      >{{ row.highlight }}</span
                    >
                  </div>
                </div>
            ...
            <td class="pr-8 py-4">
              <div class="w-max text-sm font-semibold text-[#2B3E5C]">{{ row.price }} ₾</div>
            </td>
            <td class="pr-8 py-4">
              <div class="w-max text-sm font-semibold text-[#2B3E5C]">{{ row.quantity }}</div>
            </td>
            <td class="pr-8 py-4">
              <div class="w-max text-sm font-semibold text-[#2B3E5C]">{{ row.status }}</div>
            </td>
            <td class="pr-8 py-4 text-sm font-semibold text-[#2B3E5C] w-max">
              <div class="flex flex-col gap-1 items-start">
                ...
                <div class="text-xs font-medium -tracking-wide text-[#162a4c70]">
                  {{ row.updatedAt }}
                </div>
              </div>
            </td>
            ...
            >
              <div class="flex items-center justify-center gap-2">
                <div
                  class="size-[40px] cursor-pointer rounded-full bg-[#EBF3FF] flex justify-center items-center"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    version="1.1"
                ...
                <div
                  class="size-[40px] cursor-pointer rounded-full bg-[#F1F2F4] flex justify-center items-center"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    version="1.1"
